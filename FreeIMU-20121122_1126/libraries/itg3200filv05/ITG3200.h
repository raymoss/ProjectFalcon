...
#define True 1
#define False 0
#define MYASSERT(a,message) if(a<0){ usart_printfm(USARTx,(const int *)message); return -1;} 
...
public:
  ...
  int setSampleRateDiv(byte _SampleRate);
  // Digital Low Pass Filter BandWidth and SampleRate 
  byte getFSRange();
  int setFSRange(byte _Range); // RANGE2000
  byte getFilterBW(); 
  int setFilterBW(byte _BW); // see register parameters above
  ...
  // INT pin triggers
  bool isITGReadyOn();          
  int setITGReady(bool _State);
  bool isRawDataReadyOn();
  int setRawDataReady(bool _State);      
  // Trigger Status
  bool isITGReady();
  bool isRawDataReady();
  // Gyro Sensors
  void readTemp(float *_Temp);
  int readGyroRaw(int  *_GyroXYZ);
  int readGyroRaw(int *_GyroX, int *_GyroY, int *_GyroZ);
  ...
  byte getClockSource();
  int setClockSource(byte _CLKsource); // see register parameters above
